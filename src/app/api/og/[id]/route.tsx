...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return new ImageResponse(
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between", padding: 70, background: "#f6f7f5", color: "#17201a", fontFamily: "Arial, sans-serif" }}>
        ...
        <div style={{ display: "flex", gap: 18, marginTop: 42 }}>
          {stats.map((stat) => (
            <div key={stat.label} style={{ minWidth: 200, display: "flex", flexDirection: "column", padding: "22px 26px", borderRadius: 14, background: "#e0f2e8" }}>
              <span style={{ color: "#0d4d30", fontSize: 40, fontWeight: 800 }}>{stat.value}</span>
